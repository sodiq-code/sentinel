...
async function main() {
  ...
  const issue = await openIssue({
    ...
    body: [
      ...
      '',
      '**Sentinel will NEVER merge or close this issue.** A human reviewer must close it (no-merge policy).',
      '',
  ...
  try {
    prResult = await openPR({
      ...
      body: [
        ...
        'at 1h stale (not 6h).',
        '',
        '**Sentinel will NEVER merge this PR.** A human reviewer must merge it (no-merge policy).',
        '',
        `— Sentinel Agent (incident ${incidentUrn})`,
      ].join('\n'),
