...
//   - traceLogPath(kind) — `examples/trace/{kind}-actions.log` JSONL
//   - appendTraceLog(kind, record) — append-only JSONL; mkdir -p the dir.
//   - readTraceLog(kind, limit) — read the last N entries (newest last).
//
// Trace log files are gitignored (Phase 3 .gitignore update), so live demo
...
export function traceLogPath(kind: 'github' | 'slack'): string {
  return path.join(process.cwd(), 'examples', 'trace', `${kind}-actions.log`)
}
...
/** Read the last N trace log entries (newest last). Returns [] if file missing. */
